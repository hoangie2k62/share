...
@WebServlet(urlPatterns = {"/ad/quanlychi"})
public class QuanLy_Chi extends HttpServlet{
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
    	...
    	try {
          ...
          ArrayList<ThongTin> list = DBUtilsSpend.querySpending(conn, date1, date2, word);
    	  
    	  request.setAttribute("departments", list); 
    	  
          ArrayList<Pair> list2 = DBUtils.veBD(conn, date1, date2);
          
    	  request.setAttribute("Pair", list2); 

	      RequestDispatcher dispatcher = this.getServletContext().getRequestDispatcher("/demo/views/admin/quanli_chi.jsp");
